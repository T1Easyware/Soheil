...
namespace Soheil.Controls.Convertors
{
    public class MarginConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values.Length == 3)
            {
                Thickness marginFactor = values[0] is Thickness ? (Thickness) values[0] : new Thickness();
                double width = values[1] is double ? (double) values[1] : 0;
                double height = values[2] is double ? (double) values[2] : 0;

                return new Thickness(width*marginFactor.Left, height*marginFactor.Top, width*marginFactor.Right,
                                     height*marginFactor.Bottom);
            }
