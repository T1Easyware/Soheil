...
namespace Soheil.Common
{
	...
	public static class ProjectionEqualityComparer
	{
		...
		/// after being projected from the elements</typeparam>
		/// <param name="projection">Projection to use when determining the key of an element</param>
		/// <param name="throwOnNull">throws if any of 2 objects is null during compare</param>
		/// <returns>A comparer which will compare elements by projecting 
		/// each element to its key, and comparing keys</returns>
		public static ProjectionEqualityComparer<TSource, TKey> Create<TSource, TKey>(Func<TSource, TKey> projection, bool throwOnNull)
		{
			return new ProjectionEqualityComparer<TSource, TKey>(projection) { Throw = true };
		}
	...
	public class ProjectionEqualityComparer<TSource, TKey> : IEqualityComparer<TSource>
	{
		...
		public int GetHashCode(TSource obj)
		{
			if (obj == null)
			{
				if (Throw) throw new ArgumentNullException("obj");
			}
