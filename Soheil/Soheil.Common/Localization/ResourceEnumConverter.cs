...
namespace Soheil.Common.Localization
{
    ...
    /// project.
    /// </remarks>
    public class ResourceEnumConverter : EnumConverter, INotifyPropertyChanged
    {
        ...
        private string GetValueText(CultureInfo culture, object value)
        {
            ...
            string resourceName = string.Format("{0}_{1}", type.Name, value);
            string result = _resourceManager.GetString(resourceName, culture) ?? resourceName;
            return result;
        }
        ...
        private string GetFlagValueText(CultureInfo culture, object value)
        {
            ...
            return (from object flagValue in _flagValues let lFlagValue = Convert.ToUInt32(flagValue) where IsSingleBitValue(lFlagValue) where (lFlagValue & lValue) == lFlagValue select GetValueText(culture, flagValue)).Aggregate<string, string>(null, (current, valueText) => current == null ? valueText : string.Format("{0}, {1}", current, valueText));
        }
        ...
        {
            culture = Dispatcher.CurrentDispatcher.Thread.CurrentCulture;
            if (value is string)
            {
        ...
        static public List<KeyValuePair<Enum, string>> GetValues(Type enumType, CultureInfo culture)
        {
            ...
            return (
                from Enum value in Enum.GetValues(enumType) 
                    select new KeyValuePair<Enum, string>(value, converter.ConvertToString(null, culture, value))).ToList();
        }
