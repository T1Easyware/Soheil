...
{
    public class BooleanToLengthConverter : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            if (values.Length == 2)
            {
                if (values[0] is Boolean && values[1] is double)
                {
                    return (Boolean) values[0] ? 0 : values[1];
                }
                if (values[0] is double && values[1] is Boolean)
                {
                    return (Boolean) values[1] ? 0 : values[0];
                }
            }
            return 0;
        }
