...
namespace Soheil.Core.DataServices
{
    public class HolidayDataService : DataServiceBase, IDataService<Holiday>
	{
        ...
        Repository<Holiday> holidayRepository;
        SoheilEdmContext _context;
        SoheilEdmContext context { get { if (_context == null) _context = new SoheilEdmContext(); return _context; } set { _context = context; } }

		public HolidayDataService()
		{
			context = new SoheilEdmContext();
			holidayRepository = new Repository<Holiday>(context);
		}
		...
		{
            return holidayRepository.Single(x => x.Id == id);
		}
        ...
        public System.Collections.ObjectModel.ObservableCollection<Holiday> GetAll()
		{
            ...
            var items = holidayRepository.GetAll();
		...
		{
            holidayRepository.Delete(model);
			context.Commit();
		}
		...
		{
            var model = holidayRepository.Single(x => x.Id == id);
        ...
        {
            return holidayRepository.FirstOrDefault(x => x.Date == DateTime.Now.Date);
        }
        ...
        internal List<Holiday> GetAtDate(DateTime date)
        {
            ...
            var d = date.Date;
			result.AddRange(holidayRepository.Find(x => 
				x.Date == d)
				.ToList());
			result.AddRange(holidayRepository.Find(x =>
				x.IsRecurrent &&
